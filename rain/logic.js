...
let loop = true
let scrW, scrH, lineMaxLength, lineMinLength, lengths, numberOfXPositions, numberOfDrops, numberOfYPositions, yPositions

function calculateParams() {
  scrW = window.innerWidth
  scrH = window.innerHeight
  numberOfXPositions = parseInt(scrW / letterWidth)
  numberOfDrops = Math.floor(numberOfXPositions / 3)
  ...
  // console.log(`scrW: ${scrW} | scrH: ${scrH} | lineMaxLength: ${lineMaxLength} | lineMinLength: ${lineMinLength} | numberOfXPositions: ${numberOfXPositions} | numberOfDrops: ${numberOfDrops} | numberOfYPositions: ${numberOfYPositions}`)
}
...
class Drop {
  ...
  fall() {
    ...
    // const text = setCharAt(this.line.innerHTML, 0, asciiUppercase[Math.floor(Math.random() * asciiUppercase.length)])
    // this.line.innerText = text.substring(1, text.length) + text[0]
